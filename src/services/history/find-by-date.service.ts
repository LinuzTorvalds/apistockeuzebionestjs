...
export class FindByDateHistoryService {
  ...
  async findByDate(history: FindByDateHistory) {
    const response: FindByDateHistoryResponse[] = await this.prisma.$queryRaw<
      FindByDateHistoryResponse[]
    >`select * from history where date_of_use between 
    ${history.start_date} and ${history.end_date} order by description`.finally(
      async () => await this.prisma.$disconnect()
    )
    return response
  }
}
